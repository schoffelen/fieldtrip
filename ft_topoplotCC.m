...
cfg.linestyle   = ft_getopt(cfg, 'linestyle',   'straight');
cfg.colormap    = ft_getopt(cfg, 'colormap',    colormap);
cfg.renderer    = ft_getopt(cfg, 'renderer',    []);      % let MATLAB decide on the default
cfg.curveparam  = ft_getopt(cfg, 'curveparam',  0.8);
...
% deal with divided edge bundling a bit differently, this needs the graph
% to be processed as a whole
if strcmp(cfg.linestyle, 'divedge')
  keyboard
...
for i=srtidx(:)'
  ...
  if widthparam(i)>0 && (isempty(alphaparam)||alphaparam(i)>0)
    ...
    switch cfg.linestyle
      ...
      case 'curve'
        tmp = cscvn([xbeg mean([xbeg,xend])*cfg.curveparam xend;ybeg mean([ybeg,yend])*cfg.curveparam yend]);
        pnt = fnplt(tmp);
        h   = line(pnt(1,:)', pnt(2,:)');
